...
function broadcastRoomList(io) {
    ...
    io.of("/spectator").emit("roomListUpdated", openRooms);

    const counts = getRoomCounts();
    io.emit("roomCountsUpdated", counts);
    io.of("/spectator").emit("roomCountsUpdated", counts);
}

function getRoomCounts() {
    const allRooms = [...rooms.values()];
    const waiting = allRooms.filter((r) => r.status === "waiting").length;
...
function roomSocketHandler(io, socket) {
    ...
    socket.on(
        "createRoom",
        async ({ boardSize, boardStyle, marker1, customBoardImage }) => {
            ...
            if (existingRoom) {
                socket.emit("joinError", {
                    message: "You are currently in a room",
                });
                return;
            }
        ...
        async ({ boardSize, boardStyle, marker1, customBoardImage }) => {
            ...
            if (availableRoom) {
                ...
                io.to(availableRoom.roomCode).emit("waitForStart", {
                    ...
                    player2Name: socket.user.username,
                    player1AvatarURL:
                        availableRoom.player1User.avatarURL || null,
                    player2AvatarURL: socket.user.avatarURL || null,
                    player2SocketId: socket.id,
                });
    ...
    socket.on("startGame", async ({ roomCode, marker2 }) => {
        ...
        try {
            ...
            io.to(normalizedCode).emit("joinError", {
    ...
    socket.on("leaveOnlineMatch", async ({ roomCode, sessionId }) => {
        ...
        if (!room || !isRoomPlayer(room, socket.id)) {
            socket.emit("roomLifecycleError", {
                message: "Online room not found.",
            });
            return;
        }
        ...
        if (
            sessionId &&
            room.sessionId &&
            String(sessionId) !== String(room.sessionId)
        ) {
            socket.emit("roomLifecycleError", {
                message: "Session does not match this room.",
            });
            return;
        }

        await terminateActiveRoom(
            io,
            normalizedCode,
            room,
            socket.id,
    ...
    socket.on("requestRematch", ({ roomCode, sessionId }) => {
        ...
        if (
            !["finished_waiting_decision", "rematch_pending"].includes(
                room.status,
            )
        ) {
            socket.emit("roomLifecycleError", {
                message: "This room is not ready for a rematch.",
            });
            return;
        }
    ...
    socket.on("respondRematch", ({ roomCode, accept }) => {
        ...
        if (room.status !== "rematch_pending" || !room.rematchRequestedBy) {
            socket.emit("roomLifecycleError", {
                message: "No rematch request is pending.",
            });
            return;
        }
        ...
        for (const [code, room] of rooms) {
            ...
            if (
                ...
                await closeRoomAfterPostMatch(io, code, room, {
                    ...
                    message:
                        "Your opponent left the room. This room has been closed.",
                });
                continue;
            }
...
roomSocketHandler.transitionRoomToFinishedDecision =
    transitionRoomToFinishedDecision;
roomSocketHandler.getRoomCounts = getRoomCounts;
