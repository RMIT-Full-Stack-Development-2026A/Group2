...
export default function useGamePlayView(config) {
    ...
    useEffect(() => {
        ...
        socket.on("roomClosed", ({ roomCode, reason }) => {
            ...
            const myRoom = String(config?.roomCode || "").toUpperCase();
            if (closedRoom !== myRoom) return;

            const message =
                reason === "closed_by_admin"
                    ? `Room ${closedRoom} was closed by an admin.`
                    : `Room ${closedRoom} was closed.`;
    ...
    const handleBackendCellClick = useCallback(
        async (row, col) => {
            if (!sessionId) return;
            if (winner || aborted || aiThinking || isMovePending || isPaused)
                return;
            ...
            try {
                ...
                const marker =
                    currentPlayer === 1 ? config.marker1 : config.marker2;
    ...
    const handleCellClick = useCallback(
        (row, col) => {
            if (isOnline) {
                ...
                const myMarker =
                    myRole === "player1" ? config.marker1 : config.marker2;
